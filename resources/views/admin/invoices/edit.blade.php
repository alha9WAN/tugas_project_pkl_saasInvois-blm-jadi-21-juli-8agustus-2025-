    ...
    <div class="container py-3">
        <div class="card shadow-lg">
            <div class="card-header bg-primary text-white">
                <h3 class="mb-0"><i class="fas fa-file-invoice me-2"></i> Edit Invoice </h3>
            </div>
            <div class="card-body">
                <form id="invoiceForm" action="/admin/invoices/{{ $data_invoice->id}}" method="POST">
                    @csrf
                      @method('PUT')
                    <!-- Header Invoice -->
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <div class="mt-1">
                                ...
                                <select name="client_id" class="form-select" required>
                                    @foreach ($clients as $client)
                                        <option value="{{ $client->id }}"
                                            {{ $data_invoice->client_id == $client->id ? 'selected' : '' }}>
                                            {{ $client->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                {{-- <select class="form-select" id="client_id" name="client_id" required>
                                    ...
                                    @foreach ($data_invois as $item)
                                        <option value="{{ $data_invois->$client->id }}">{{ $data_invois->$client->name }}</option>
                                    @endforeach
                                </select> --}}
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered">
                                    ...
                                    <tr class="table-active">
                                        <th>Total</th>
                                        <td id="total">{{ $data_invoice->total }}</td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                    ...
                    <div class="row mt-4">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="status" name="status">
                                    @foreach ($statusOptions as $key => $label)
                                        <option value="{{ $key }}"
                                            {{ old('status', $invoice->status ?? '') == $key ? 'selected' : '' }}>
                                            {{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-9 text-end">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i> Edit Invoice
                            </button>
                        </div>
                    </div>
                </form>
